...
class PathConfig:
    ...
    def __init__(self, dir_archive):
        ...
        self.example_file_output_tweets = self.create_path_for_file_output_tweets(
            year=2020, month=12
        )

    def create_path_for_file_output_tweets(
        self, year, month, format="html", kind="tweets"
    ) -> str:
        ...
        return os.path.join(
            ...
            f"{kind}-{format}",
            f"{year:04}",
            f"{year:04}-{month:02}-01-{kind}.{format}",
        )
    ...
    def create_path_for_file_output_dms(
        ...
        return os.path.join(
            self.dir_output, kind, f"{kind}-{name}{index_suffix}.{format}"
        )
...
def get_consent(prompt: str, default_to_yes: bool = False):
    ...
    while True:
        ...
        if user_input == "":
            print(
                f"Your empty response was assumed to mean '{default_answer}' (the default for this question)."
            )
        ...
        if user_input.lower() in ("n", "no"):
            return False
        print(
            f"Sorry, did not understand. Please answer with y, n, yes, no, or press enter to accept "
...
def get_twitter_users(session, bearer_token, guest_token, user_ids):
    ...
    while user_ids:
        ...
        query_url = (
            f"https://api.twitter.com/1.1/users/lookup.json?user_id={user_id_list}"
        )
...
def lookup_users(user_ids, users):
    ...
    if not get_consent(
        f"Download user data from Twitter (approx {estimated_size:,} KB)?"
    ):
        return
    ...
    try:
        with requests.Session() as session:
            ...
            retrieved_users = get_twitter_users(
                session, bearer_token, guest_token, filtered_user_ids
            )
            for user_id, user in retrieved_users.items():
                if user["screen_name"] is not None:
                    users[user_id] = UserData(
                        user_id=user_id, handle=user["screen_name"]
                    )
...
def convert_tweet(tweet, username, media_sources, users: dict, paths: PathConfig):
    ...
    timestamp = int(
        round(
            datetime.datetime.strptime(timestamp_str, "%a %b %d %X %z %Y").timestamp()
        )
    )
    ...
    if (
        "entities" in tweet
        and "media" not in tweet["entities"]
        and len(tweet["entities"].get("urls", [])) == 0
    ):
        for word in tweet["full_text"].split():
            try:
                ...
                if (
                    ...
                    netloc_short = (
                        url.netloc[4:] if url.netloc.startswith("www.") else url.netloc
                    )
                    path_short = (
                        url.path
                        if len(url.path + "?" + url.query) < 15
                    ...
                    tweet["entities"]["urls"].append(
                        {
                            ...
                            "indices": [
                                tweet["full_text"].index(word),
                                tweet["full_text"].index(word) + len(word),
                            ],
                        }
                    )
    ...
    if (
        ...
        and "extended_entities" in tweet
        ...
        for media in tweet["extended_entities"]["media"]:
            if "url" in media and "media_url" in media:
                ...
                archive_media_path = os.path.join(
                    paths.dir_input_media, archive_media_filename
                )
                file_output_media = os.path.join(
                    paths.dir_output_media, archive_media_filename
                )
                media_url = rel_url(file_output_media, paths.example_file_output_tweets)
                markdown += (
                    ""
                    if not markdown and body_markdown == escape_markdown(original_url)
                    else "\n\n"
                )
                ...
                if os.path.isfile(archive_media_path):
                    ...
                    best_quality_url = (
                        f"https://pbs.twimg.com/media/{original_filename}:orig"
                    )
                    ...
                    archive_media_paths = glob.glob(
                        os.path.join(paths.dir_input_media, tweet_id_str + "*")
                    )
                    if len(archive_media_paths) > 0:
                        for archive_media_path in archive_media_paths:
                            archive_media_filename = os.path.split(archive_media_path)[
                                -1
                            ]
                            file_output_media = os.path.join(
                                paths.dir_output_media, archive_media_filename
                            ...
                            media_url = rel_url(
                                file_output_media, paths.example_file_output_tweets
                            )
                            if not os.path.isfile(file_output_media):
                                shutil.copy(archive_media_path, file_output_media)
                            markdown += (
                                f'<video controls><source src="{media_url}">Your browser '
                            ...
                            if (
                                ...
                                best_bitrate = (
                                    -1
                                )  # some valid videos are marked with bitrate=0 in the JSON
    ...
    if (
        "entities" in tweet
        and "user_mentions" in tweet["entities"]
        and tweet["entities"]["user_mentions"] is not None
    ):
            ...
            if mention is not None and "id" in mention and "screen_name" in mention:
                ...
                if int(mentioned_id) >= 0:  # some ids are -1, not sure why
                    ...
                    if handle is not None:
                        users[mentioned_id] = UserData(
                            user_id=mentioned_id, handle=handle
                        )
...
def find_files_input_tweets(dir_path_input_data):
    ...
    for input_tweets_file_template in input_tweets_file_templates:
        files_paths_input_tweets += glob.glob(
            os.path.join(dir_path_input_data, input_tweets_file_template)
        )
    if len(files_paths_input_tweets) == 0:
        print(
            f"Error: no files matching {input_tweets_file_templates} in {dir_path_input_data}"
        )
        exit()
...
def find_dir_input_media(dir_path_input_data):
    ...
    for input_media_dir_template in input_media_dir_templates:
        input_media_dirs += glob.glob(
            os.path.join(dir_path_input_data, input_media_dir_template)
        )
    if len(input_media_dirs) == 0:
        print(
            f"Error: no folders matching {input_media_dir_templates} in {dir_path_input_data}"
        )
        exit()
    if len(input_media_dirs) > 1:
        print(
            f"Error: multiple folders matching {input_media_dir_templates} in {dir_path_input_data}"
        )
        exit()
    ...
    try:
        with requests.get(url, stream=True, timeout=2) as res:
            if not res.status_code == 200:
                ...
                raise Exception(
                    f'Download failed with status "{res.status_code} {res.reason}". '
            ...
            if byte_size_after != byte_size_before:
                ...
                pixels_percentage_increase = (
                    100.0 * (pixels_after - pixels_before) / pixels_before
                )
                ...
                if (
                    ...
                    bytes_percentage_increase = (
                        100.0 * (byte_size_after - byte_size_before) / byte_size_before
                    )
                    logging.info(
                        f"{pref}SUCCESS. New version is {bytes_percentage_increase:3.0f}% "
                    ...
                    logging.info(
                        f"{pref}SKIPPED. Pixel size comparison inconclusive: "
                    ...
                    if bytes_percentage_increase >= 0:
                        logging.info(
                            f"{pref}SUCCESS. New version is {bytes_percentage_increase:3.0f}% larger in bytes "
                            f"and {pixels_percentage_increase:3.0f}% larger in pixels. {post}"
                        )
                    else:
                        logging.info(
                            f"{pref}SUCCESS. New version is actually {-bytes_percentage_increase:3.0f}% "
                    ...
                    logging.info(
                        f"{pref}SKIPPED. Online version has {-pixels_percentage_increase:3.0f}% "
                        f"smaller pixel size. {post}"
                    )
                    return True, byte_size_after
            else:
                logging.info(
                    f"{pref}SKIPPED. Online version is same byte size, assuming same content. Not downloaded."
                )
                return True, 0
    except Exception as err:
        logging.error(
            f"{pref}FAIL. Media couldn't be retrieved from {url} because of exception: {err}"
        )
        return False, 0
...
def download_larger_media(media_sources, paths: PathConfig):
    ...
    while remaining_tries > 0:
        ...
        for index, (local_media_path, media_url) in enumerate(media_sources):
            ...
            if estimated_time_remaining.hour >= 1:
                ...
                time_remaining_string: str = f"{estimated_time_remaining.second} second{'' if estimated_time_remaining.second == 1 else 's'}"

            if index + 1 == number_of_files:
                print("    100 % done.")
            else:
                print(
                    f"    {(100*(index+1)/number_of_files):.1f} % done, about {time_remaining_string} remaining..."
                )
    ...
    logging.info(
        f"Total downloaded: {total_bytes_downloaded/2**20:.1f}MB = {total_bytes_downloaded/2**30:.2f}GB"
    )
...
def collect_user_ids_from_followings(paths) -> list:
    ...
    following_json = read_json_from_js_file(
        os.path.join(paths.dir_input_data, "following.js")
    )
...
def parse_followings(users, user_id_url_template, paths: PathConfig):
    ...
    for following_id in following_ids:
        handle = (
            users[following_id].handle if following_id in users else "~unknown~handle~"
        )
        following.append(handle + " " + user_id_url_template.format(following_id))
    following.sort()
    following_output_path = paths.create_path_for_file_output_single(
        format="txt", kind="following"
    )
...
def collect_user_ids_from_followers(paths) -> list:
    ...
    """
    # read JSON file from archive
    follower_json = read_json_from_js_file(
        os.path.join(paths.dir_input_data, "follower.js")
    )
    # collect all user ids in a list
    follower_ids = []
...
def parse_followers(users, user_id_url_template, paths: PathConfig):
    ...
    for follower_id in follower_ids:
        handle = (
            users[follower_id].handle if follower_id in users else "~unknown~handle~"
        )
        followers.append(handle + " " + user_id_url_template.format(follower_id))
    followers.sort()
    followers_output_path = paths.create_path_for_file_output_single(
        format="txt", kind="followers"
    )
...
def collect_user_ids_from_direct_messages(paths) -> list:
    ...
    """
    # read JSON file from archive
    dms_json = read_json_from_js_file(
        os.path.join(paths.dir_input_data, "direct-messages.js")
    )
    # collect all user ids in a set
    dms_user_ids = set()
    for conversation in dms_json:
        if (
            "dmConversation" in conversation
            and "conversationId" in conversation["dmConversation"]
        ):
...
def parse_direct_messages(username, users, user_id_url_template, paths: PathConfig):
    ...
    for conversation in dms_json:
        if (
            ...
            if "messages" in dm_conversation:
                for message in dm_conversation["messages"]:
                    if "messageCreate" in message:
                        ...
                        if all(
                            tag in message_create
                            for tag in ["senderId", "recipientId", "text", "createdAt"]
                        ):
                            ...
                            # replace t.co URLs with their original versions
                            if (
                                "urls" in message_create
                                and len(message_create["urls"]) > 0
                            ):
                            ...
                            # replace image URLs with image links to local files
                            if (
                                "mediaUrls" in message_create
                                and len(message_create["mediaUrls"]) == 1
                                and "urls" in message_create
                            ):
                                original_expanded_url = message_create["urls"][0][
                                    "expanded"
                                ]
                                message_id = message_create["id"]
                                media_hash_and_type = message_create["mediaUrls"][
                                    0
                                ].split("/")[-1]
                                media_id = message_create["mediaUrls"][0].split("/")[-2]
                                archive_media_filename = (
                                    f"{message_id}-{media_hash_and_type}"
                                )
                                new_url = os.path.join(
                                    paths.dir_output_media, archive_media_filename
                                )
                                archive_media_path = os.path.join(
                                    paths.dir_input_data,
                                ...
                                if os.path.isfile(archive_media_path):
                                    ...
                                    body_markdown = body_markdown.replace(
                                        escape_markdown(original_expanded_url),
                                        image_markdown,
                                    )
                                    ...
                                    if len(archive_media_paths) > 0:
                                        for archive_media_path in archive_media_paths:
                                            archive_media_filename = os.path.split(
                                                archive_media_path
                                            )[-1]
                                    ...
                                    #  (see above)
                                        ...
                                        print(
                                            f"Warning: missing local file: {archive_media_path}. "
                                            f"Using original link instead: {original_expanded_url})"
                                        )

                            created_at = message_create[
                                "createdAt"
                            ]  # example: 2022-01-27T15:58:52.744Z
                            timestamp = int(
                                round(
                                    datetime.datetime.strptime(
                                        created_at, "%Y-%m-%dT%X.%fZ"
                                    ).timestamp()
                                )
                            )

                            from_handle = (
                                escape_markdown(users[from_id].handle)
                                if from_id in users
                                else user_id_url_template.format(from_id)
                            )
                            to_handle = (
                                escape_markdown(users[to_id].handle)
                                if to_id in users
                            ...
                            # make the body a quote
                            body_markdown = "> " + "\n> ".join(
                                body_markdown.splitlines()
                            )
                            message_markdown = (
                                f"{from_handle} -> {to_handle}: ({created_at}) \n\n"
                                f"{body_markdown}"
                            )
            ...
            other_user_id = (
                user2_id
                if (user1_id in users and users[user1_id].handle == username)
                else user1_id
            )
    ...
    for other_user_id, messages in conversations_messages.items():
        ...
        other_user_name = (
            escape_markdown(users[other_user_id].handle)
            if other_user_id in users
            else user_id_url_template.format(other_user_id)
        )

        other_user_short_name: str = (
            users[other_user_id].handle if other_user_id in users else other_user_id
        )
        ...
        if len(messages) > 1000:
            for chunk_index, chunk in enumerate(chunks(messages, 1000)):
                markdown = ""
                markdown += (
                    f"### Conversation between {escaped_username} and {other_user_name}, "
                    f"part {chunk_index+1}: ###\n\n----\n\n"
                )
                markdown += "\n\n----\n\n".join(md for _, md in chunk)
                conversation_output_path = paths.create_path_for_file_output_dms(
                    name=other_user_short_name, index=(chunk_index + 1), format="md"
                )
            ...
            conversation_output_path = paths.create_path_for_file_output_dms(
                name=other_user_short_name, format="md"
            )
        ...
        if "messages" in dm_conversation:
            for message in dm_conversation["messages"]:
                if "messageCreate" in message:
                    ...
                    for participant_id in message["joinConversation"][
...
def collect_user_ids_from_group_direct_messages(paths) -> list:
    ...
    """
    # read JSON file from archive
    group_dms_json = read_json_from_js_file(
        os.path.join(paths.dir_input_data, "direct-messages-group.js")
    )
    # collect all user ids in a set
...
def parse_group_direct_messages(username, users, user_id_url_template, paths):
            ...
            for participant_id in participants:
                if participant_id in users:
                    ...
                    participant_names.append(
                        user_id_url_template.format(participant_id)
                    )

            # save names in metadata
            group_conversations_metadata[conversation_id]["participants"] = participants
            group_conversations_metadata[conversation_id][
                "participant_names"
            ] = participant_names
            group_conversations_metadata[conversation_id]["conversation_names"] = [
                (0, conversation_id)
            ]
            group_conversations_metadata[conversation_id][
            ...
            if "messages" in dm_conversation:
                for message in dm_conversation["messages"]:
                    if "messageCreate" in message:
                        ...
                        if all(
                            tag in message_create
                            for tag in ["senderId", "text", "createdAt"]
                        ):
                            from_id = message_create["senderId"]
                        ...
                        if all(
                            tag in conversation_name_update
                            for tag in ["initiatingUserId", "name", "createdAt"]
                        ):
                            ...
                            created_at = conversation_name_update[
                                "createdAt"
                            ]  # example: 2022-01-27T15:58:52.744Z
                            timestamp = int(
                                round(
                            ...
                            group_conversations_metadata[conversation_id][
                                "conversation_names"
                            ].append((timestamp, conversation_name_update["name"]))
                    elif "joinConversation" in message:
                        join_conversation = message["joinConversation"]
                        if all(
                            tag in join_conversation
                            for tag in ["initiatingUserId", "createdAt"]
                        ):
                            from_id = join_conversation["initiatingUserId"]
                            created_at = join_conversation[
                                "createdAt"
                            ]  # example: 2022-01-27T15:58:52.744Z
                            timestamp = int(
                                round(
                            ...
                            escaped_username = escape_markdown(username)
                            body_markdown = (
                                f"_{from_handle} added {escaped_username} to the group_"
                            )
                            message_markdown = (
                                f"{from_handle}: ({created_at})\n\n{body_markdown}"
                            )
                        ...
                        if all(
                            tag in participants_join
                            for tag in ["initiatingUserId", "userIds", "createdAt"]
                        ):
                            from_id = participants_join["initiatingUserId"]
                            created_at = participants_join[
                                "createdAt"
                            ]  # example: 2022-01-27T15:58:52.744Z
                            timestamp = int(
                                round(
                            ...
                            joined_handles = [
                                escape_markdown(users[joined_id].handle)
                                if joined_id in users
                                else user_id_url_template.format(joined_id)
                                for joined_id in joined_ids
                            ]
                            name_list = ", ".join(joined_handles[:-1]) + (
                                f" and {joined_handles[-1]}"
                                if len(joined_handles) > 1
                                else joined_handles[0]
                            )
                            body_markdown = (
                                f"_{from_handle} added {name_list} to the group_"
                            )
                            message_markdown = (
                                f"{from_handle}: ({created_at})\n\n{body_markdown}"
                            )
                        ...
                        if all(
                            tag in participants_leave
                            for tag in ["userIds", "createdAt"]
                        ):
                            created_at = participants_leave[
                                "createdAt"
                            ]  # example: 2022-01-27T15:58:52.744Z
                            timestamp = int(
                                round(
                            ...
                            left_handles = [
                                escape_markdown(users[left_id].handle)
                                if left_id in users
                                else user_id_url_template.format(left_id)
                                for left_id in left_ids
                            ]
                            name_list = ", ".join(left_handles[:-1]) + (
                                f" and {left_handles[-1]}"
                                if len(left_handles) > 1
                                else left_handles[0]
                            )
                            body_markdown = f"_{name_list} left the group_"
                            message_markdown = (
                                f"{name_list}: ({created_at})\n\n{body_markdown}"
                            )
                            messages.append((timestamp, message_markdown))
    ...
    for conversation_id, messages in group_conversations_messages.items():
        ...
        group_conversations_metadata[conversation_id]["conversation_names"].sort(
            key=lambda tup: tup[0], reverse=True
        )
        official_name = group_conversations_metadata[conversation_id][
            "conversation_names"
        ][0][1]
        ...
        if group_name == conversation_id:
            ...
            handles = []
            for participant_id, message_count in group_conversations_metadata[
                conversation_id
            ...
            if len(handles) == 1:
                group_name = f'{handles[0][0]}_and_{len(group_conversations_metadata[conversation_id]["participants"]) - 1}_more'
            elif (
                len(handles) == 2
                and len(group_conversations_metadata[conversation_id]["participants"])
                == 3
            ):
        ...
        escaped_participant_names = [
            ...
            for participant_name in group_conversations_metadata[conversation_id][
                "participant_names"
            ]
        ]
        ...
        if len(messages) > 1000:
            ...
            markdown += "\n\n----\n\n".join(md for _, md in messages)
            conversation_output_filename = paths.create_path_for_file_output_dms(
                name=group_name, format="md", kind="DMs-Group"
            )
...
def migrate_old_output(paths: PathConfig):
    ...
    if os.path.exists(os.path.join(paths.dir_archive, "media")):
        ...
        if len(files_to_move) > 0:
            print(
                f"Moving {len(files_to_move)} files from 'media' to '{paths.dir_output_media}'"
            )
            for file_path_to_move in files_to_move:
                file_name_to_move = os.path.split(file_path_to_move)[1]
                os.rename(
                    file_path_to_move,
                    os.path.join(paths.dir_output_media, file_name_to_move),
                )
    ...
    if len(files_to_delete) > 0:
        ...
        if user_input.lower() in ("y", "yes"):
            ...
            print(
                f"Files have been deleted. New versions of these files will be generated into 'parser-output' soon."
            )
...
def find_archive():
    ...
    if script_dir != os.getcwd():
        if is_archive(script_dir):
            return script_dir
    print(
        "Archive not found in working directory or script directory.\n"
...
<body>
    ...
    collected_user_ids: list = list(
        set(collected_user_ids_without_followers).union(
            collected_user_ids_only_in_followers
        )
    )
    ...
    unknown_follower_user_ids: set = unknown_collected_user_ids.intersection(
        collected_user_ids_only_in_followers
    )
    if len(unknown_follower_user_ids) > 5000:
        ...
        estimated_max_follower_lookup_time_in_minutes = (
            len(unknown_follower_user_ids) / 3000
        )
        print(
    ...
    print(
        f"Please be aware that this script may download a lot of data, which will cost you money if you are"
    )
    print(
        f"paying for bandwidth. Please be aware that the servers might block these requests if they are too"
    )
    print(
        f"frequent. This script may not work if your account is protected. You may want to set it to public"
    )
    ...
    if get_consent("OK to start downloading?"):
        ...
        print(
            "In case you set your account to public before initiating the download, "
