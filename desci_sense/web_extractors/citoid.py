...
async def fetch_all_citations(urls: list):
    ...
    async with aiohttp.ClientSession(connector=my_conn) as session:
        tasks = []
        for url in urls:
            task = asyncio.ensure_future(fetch_citation_async(target_url=url,session=session))
            tasks.append(task)
...
def fetch_citation(target_url):
    ...
    if response.status_code == 200:
        ...
        formatted_err_msg = {"msg": f'Error: Unable to fetch data. Status code: {response.status_code}'}
